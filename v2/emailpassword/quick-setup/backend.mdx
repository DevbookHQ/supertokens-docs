...
import DevbookView from "/src/components/DevbookView"
...
<TabItem value="nodejs">
<DevbookView
    command="npm i -s supertokens-node"
/>
</TabItem>
<TabItem value="go">
...
export let nodeInitCode = '// 2) Initialise SuperTokens'
...
<CoreInjector
    askForWebsiteDomain={true}
    onChange={(code) => nodeInitCode = code}
>
...
export let nodeAPICode = '// 3) Add the SuperTokens APIs & CORS setup'
...
<CoreInjector
    onChange={(code) => nodeAPICode = code}
>
...
export let nodeErrorCode = '// 4) Add the SuperTokens error handler'
...
<TabItem value="express">
<CoreInjector
    onChange={(code) => nodeErrorCode = code}
>
